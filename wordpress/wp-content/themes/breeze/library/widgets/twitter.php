...
function parse_twitter_cache_feed($usernames, $limit, $interval) {
	...
	if ($amount) { // are there any tweets?
		...
		for ($i = 1; $i <= $amount; $i++) {
			...
			$clean_link_1 = explode("<link type=\"text/html\" rel=\"alternate\" href=\"", $entry_close[0]);
			$clean_link = explode("\"/>", $clean_link_1[1]);
			...
			?>
            	<li>
                    ...
                    <small>
						...
						<a href="<?php echo $clean_link[0]; ?>"><?php echo $pretty_time; ?></a>
                    </small>
        ...
        <ul>
        	<li>
                <p class="tweet rounded-all">
                    <?php _e( 'No tweets were found.', 'unisphere' ); ?>
                </p>
                <small>&nbsp;</small>
                <div class="tweet-arrow"></div>
            </li>			
